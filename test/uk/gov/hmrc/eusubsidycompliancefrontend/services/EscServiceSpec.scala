  ...
  "EscService" when {
    ...
    "handling request to retrieve subsidy" must {
      ...
      "return an error" when {
        ...
        def isError: Assertion = {
          service.retrieveSubsidies(undertakingRef).failed.futureValue shouldBe a[RuntimeException]
        }
      ...
      "return successfully" when {
        ...
        "the undertaking subsidies are present in the cache" in {
          ...
          service.retrieveSubsidies(undertakingRef).futureValue shouldBe undertakingSubsidies
        }

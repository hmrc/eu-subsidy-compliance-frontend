...
class ConfirmBusinessDetailsControllerSpec
  ...
  private val controller = instanceOf[ConfirmBusinessDetailsController]

  "ConfirmBusinessDetailsController" when {
    ...
    "handling submit" must {
      ...
      def performAction(data: (String, String)*) = controller.submitPage()(
        FakeRequest(POST, routes.ConfirmBusinessDetailsController.submitPage().url)
      ...
      "redirect to the email HMRC page when details are reported wrong (No)" in {
        ...
        checkIsRedirect(
          ...
          routes.EmailHMRCUpdateBusinessDetailsController.showPage().url
        )
      }
    }
  }

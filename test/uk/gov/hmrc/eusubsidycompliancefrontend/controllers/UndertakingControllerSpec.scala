...
class UndertakingControllerSpec
  ...
  "handling request to get sector in UpdateNaceMode" must {
    ...
    "transform sector code '10' to display value '00' in UpdateNaceMode" in {
      ...
      status(result) shouldBe OK
      val document = Jsoup.parse(contentAsString(result))
      document.select("input[name=undertakingSector][value=00]").hasAttr("checked") shouldBe true
    }

    "transform sector code '35.11' to display value '00' in UpdateNaceMode" in {
      val journey = UndertakingJourney(
        sector = UndertakingSectorFormPage(Sector.nonRenewableElectricityProduction.some),
        about = AboutUndertakingFormPage("Test Undertaking".some),
        mode = appConfig.UpdateNaceMode
      )
      inSequence {

...
lazy val microservice = Project(appName, file("."))
  ...
  .settings(
    ...
    ScoverageKeys.coverageExcludedFiles := "<empty>;Reverse.*;.*filters.*;.*handlers.*;.*components.*;.*repositories.*;.*config.*;" +
      ...
      ".*ControllerConfiguration;.*testonly.*;.*models.*;.*views.*;.*job.*;.*connectors.EuropaConnector;.*connectors.ProxiedHttpClient;.*controllers.test.*;.*journeys.NilReturnJourney;.*journeys.UndertakingJourney;.*util.TimeProvider;.*persistence.*;",

  ...
  def showPage: Action[AnyContent] = enrolled.async { implicit request =>
    ...
    escService
      ...
      .foldF(
        ...
        // TODO DDCYLS-8400: Replace stub data fallback with redirect to AccountController once navigation is wired
        Future.successful(Ok(needRegistrationNumberBusinessesPage(
          List(
            BusinessEntity(EORI("GB123456789001"), leadEORI = true),
            BusinessEntity(EORI("GB123456789002"), leadEORI = false)
          ),
          eori
        )))
      ) { undertaking =>
        Future.successful(Ok(needRegistrationNumberBusinessesPage(undertaking.undertakingBusinessEntity, eori)))
      }
  }

...
@Singleton
class AboutUndertakingController @Inject() (
  ...
  def firstEmptyPage: Action[AnyContent] = enrolledUndertakingJourney.async { implicit request =>
    ...
    logger.info(s"AboutUndertakingController.firstEmptyPage attempting to find the journeys next step for EORI:$eori")

...
class UndertakingController @Inject() (
          ...
          form =>
            for {
              updatedUndertaking <- store.update[UndertakingJourney] {
                                      _.map { undertakingJourney =>
                                        val updatedSector =
                                          undertakingJourney.sector.copy(value = Some(Sector(form.value.toInt)))
          ...
          form =>
            for {
              updatedUndertaking <- store.update[UndertakingJourney] {
                                      _.map { undertakingJourney =>
                                        val updatedContact =
                                          undertakingJourney.contact.copy(value = Some(form.toContactDetails))
        ...
        form =>
          for {
            updatedJourney   <- store.update[UndertakingJourney] {
                                  _.map { undertakingJourney =>
                                    undertakingJourney
                                      .copy(cya = undertakingJourney.cya.copy(value = Some(form.value.toBoolean)))
                                  }
                                }
            undertakingName   =
              UndertakingName(updatedJourney.name.value.getOrElse(handleMissingSessionData("Undertaking Name")))
  ...
  def getAmendUndertakingDetails: Action[AnyContent] = escAuthentication.async { implicit request =>
    ...
    store.get[UndertakingJourney].flatMap {
      ensureUndertakingJourneyPresent(_) { journey =>
        for {
          updatedJourney <- if (isAmend(journey)) Future.successful(journey)
                            else
        ...
        _ =>
          for {
            ...
            retrievedUndertaking <-
              escService.retrieveUndertaking(eori).map(_.getOrElse(handleMissingSessionData("Undertaking")))
  ...
  private def getJourneyNext(journey: UndertakingJourney)(implicit request: Request[_]) =
    if (isAmend(journey)) Future.successful(Redirect(routes.UndertakingController.getAmendUndertakingDetails()))
    else journey.next
